...
function addPath(root, path, func) {
   ...
   */
   ...
   trie.setValue(root, path, function (paramValues, queryStrings, context) {
     ...
     return func(params, queryStrings, context);
   });
}

function process(root, path, opt) {
  ...
  if (route) {
    ...
    if (handler) {
      handler(route[PARAMS_INDEX], parsedPath.queryStrings, opt);
      return null;
    }
  } else {
    return 'not found';
...
function processAllHandlers(root, path) {
  ...
  if (routes) {
    routes.forEach(function (route) {
      ...
      internalContext = handler(route[PARAMS_INDEX], parsedPath.queryStrings, internalContext);
    });
  }
...
module.exports = {
  create: function () {
    ...
    return {
      ...
      process: function (path, opt) {
        return process(root, path, opt);
      },
